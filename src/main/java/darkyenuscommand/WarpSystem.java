...
final class WarpSystem {
    ...
    public boolean onCommand(Player player, String[] args){
        ...
        if("create".equalsIgnoreCase(args[0]) && args.length > 1 && !args[1].contains(":")){
            ...
            }else player.sendMessage(ChatColor.RED+"Warp "+args[1]+" does not exist!");
        }else if(("warp".equalsIgnoreCase(args[0]) || "goto".equalsIgnoreCase(args[0])) && (args.length > 1 && !args[1].contains(":"))){
            if(warps.containsKey(args[1])){
                warp(player, args[1]);
            }else player.sendMessage(ChatColor.RED+"Warp "+args[1]+" does not exist!");
        }else if("list".equalsIgnoreCase(args[0])){
            player.sendMessage(ChatColor.BLUE.toString()+ ChatColor.BOLD+"Available Warps:");
            if(warps.size() != 0)player.sendMessage(ChatColor.BLUE.toString()+Arrays.toString(warps.keySet().toArray(new String[warps.size()])));
            else player.sendMessage(ChatColor.BLUE.toString()+"None");
        }else{
