 ...
 *
 */
public class DarkyenusCommandPlugin extends JavaPlugin {
			...
			switch (command.getName()) {
				...
				case "kick":
					...
					if (toKick != null) {
						...
						sender.sendMessage(ChatColor.GREEN.toString() + toKick
								.getName() + " has been kicked for " + kickTimeMin
								+ " minutes.");
					}
					// --------------------------------------- KICK END
					return true;
						...
						if (p.match("add")) {
							...
							if (sender instanceof Player) {
								...
								}
								{
									...
									remove.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/report remove " + index + " " + reportText
											.hashCode() + " refresh " + Math
											.min(reports.size() - 1, Math.max(0, index))));
				...
				case "playerinfo":
					...
					sender.sendMessage(ChatColor.RED.toString() + ChatColor.BOLD.toString() + getInfo
							.getName() + ChatColor.BLUE
							+ "'s info");
					sender.sendMessage(ChatColor.RED + "Online: " + ChatColor.BLUE.toString() + getInfo.isOnline());
					sender.sendMessage(ChatColor.RED + "Op: " + ChatColor.BLUE.toString() + getInfo
							.isOp() + ChatColor.RED
							+ " Whitelisted: " + ChatColor.BLUE.toString() + getInfo
					...
					sender.sendMessage(ChatColor.RED + "UUID: " + ChatColor.BLUE.toString() + getInfo.getUniqueId()
							.toString());
				...
				case "darkyenuscommand":
					...
					if (args.length != 0 && "reload".equalsIgnoreCase(args[0])
						...
						sender.sendMessage(ChatColor.BLUE
								.toString() + ChatColor.ITALIC + "   (c) 2012 - 2017 darkyen@me.com");
					}
					...
					if (args.length == 0) {
						...
						if (pluginOfThatName != null) {
							...
							if (args.length > 1 || pluginOfThatName.getDescription().getCommands().size() <= 5) {
								...
								if (!beHonest) {
									strings.removeIf(commandThatMayNeedToBeHidden -> !getCommand(commandThatMayNeedToBeHidden)
								...
								sender.sendMessage(ChatColor.BLUE
										.toString() + ChatColor.ITALIC + "Use command \"/command "
										+ pluginOfThatName.getDescription().getName() + " list\" to see list of "
										+ pluginOfThatName.getDescription().getCommands()
										.size() + " commands this plugin has.");
							}
						}
				...
				case "time": {
					...
					if (world == null) {
						...
						if (world == null) {
							...
							sender.sendMessage(ChatColor.BLUE + "Time in world " + world.getName() + " is " + world
									.getTime());
						}
						return true;
					}
				...
				case "gamemode":
					...
					if ("toggle".equalsIgnoreCase(args[0]) || "t".equalsIgnoreCase(args[0])) {
						...
						if (!modeMatch.isDefinite) {
							sender.sendMessage(ChatColor.RED + "Game mode not found. " + ChatColor.BLUE + Arrays
									.toString(GameMode.values()));
							return true;
						}
					...
					sender.sendMessage(ChatColor.GRAY.toString() + ChatColor.ITALIC + "Gamemode of " + toSet
							.getName() + " set to "
							+ gameMode.toString());
					// --------------------------------------- GAMEMODE END
					return true;
				case "item":
					...
					if (params.eof()) {
						if (sender instanceof Player) {
							...
							if (item == null) {
								...
								for (Map.Entry<Enchantment, Integer> entry : item.getEnchantments().entrySet()) {
									sender.sendMessage(ChatColor.BLUE.toString() + " " + entry.getKey()
						...
						final MaterialSpec materialSpec = matchMaterialData(params.take(""), sender);
						...
						final Player toPlayer = params
								.matchPlayer(sender instanceof Player ? (Player) sender : null, sender);
						...
						if (!nbtSource.isEmpty()) {
							...
							try {
								...
								sender.sendMessage(ChatColor.RED + ("Malformed json: " + e.getMessage())
										.replace("\n", "\n" + ChatColor.RED));
								return false;
							}
							if (!nbtJson.isObject()) {
							...
							try {
								...
								sender.sendMessage(ChatColor.RED + ("Json could not be transformed to NBT: " + e
										.getMessage()).replace("\n", "\n" + ChatColor.RED));
								return false;
							}
						} else {
						...
						while (remainingToGive > 0) {
							final int itemStackSize = Math
									.min(materialSpec.material.getMaxStackSize(), remainingToGive);
				...
				case "repair": {
					...
					if (player.getInventory().getItemInMainHand().getType().getMaxDurability() != 0) {
						...
						sender.sendMessage(ChatColor.RED.toString() + player.getInventory().getItemInMainHand()
				...
				case "trash": {
					...
					if ("item".equalsIgnoreCase(args[0]) || "single".equalsIgnoreCase(args[0])) {
						...
						sender.sendMessage(ChatColor.GRAY.toString() + ChatColor.ITALIC + player
								.getName() + "'s held item trashed.");
					} else if ("hotbar".equalsIgnoreCase(args[0]) || "actionbar".equalsIgnoreCase(args[0])) {
						for (int i = 0; i <= 8; i++) {
							player.getInventory().clear(i);
						}
						sender.sendMessage(ChatColor.GRAY.toString() + ChatColor.ITALIC + player
								.getName() + "'s hotbar trashed.");
					} else if ("inventory".equalsIgnoreCase(args[0])) {
						for (int i = 0; i <= 35; i++) {
							player.getInventory().clear(i);
						}
						sender.sendMessage(ChatColor.GRAY.toString() + ChatColor.ITALIC + player
								.getName() + "'s inventory trashed.");
				...
				case "difficulty": {
					...
					if (!difficultyMatch.isDefinite) {
						sender.sendMessage(ChatColor.RED + "Difficulty not found. " + ChatColor.BLUE + Arrays
								.toString(Difficulty.values()));
						return true;
					}
				...
				case "clear": {
					...
					} else if (args[0].toLowerCase().startsWith("mob") || args[0].toLowerCase()
							.startsWith("creature")) {
				...
				case "jail":
					...
					if (matchingWarps != null && matchingWarps.isDefinite) {
						...
						if (availableWarps.results.length == 0) {
							...
							sender.sendMessage(ChatColor.GREEN.toString() + toJail
									.getName() + " jailed in " + availableWarps.results[jailID]);
						}
					}
					// --------------------------------------- JAIL END
					return true;
				...
				case "spawnentity": {
					...
					EntityType type;
					int amount = 1;
					Location at = ((Player) sender).getTargetBlock((Set<Material>) null, 200).getLocation()
							.add(0.5, 1, 0.5);
					if (at == null) {
				...
				case "world":
					...
					try {
						if (!args[0].toLowerCase().startsWith("l")) {// List
								...
								if (world == null) {
									...
									if (player.getWorld().equals(world)) {
										if (player == sender) {
											...
											sender.sendMessage(ChatColor.BLUE + "Player \"" + player
													.getName() + "\" already is in world \"" + worldName + "\".");
										}
										return true;
									}
							...
							} else if (args[0].toLowerCase().startsWith("c") || args[0].toLowerCase()
									.startsWith("d")) {// If create or remove
								if (sender.hasPermission("darkyenuscommand.command.world.manage")) {
									if (args[0].toLowerCase().startsWith("c")) {// Create world
										getServer()
												.broadcastMessage(ChatColor.GREEN + "Creating a new world, this may lag a bit.");
						...
						if (sender instanceof Player) {
							sender.sendMessage(ChatColor.BLUE + "You are in world \"" + ((Player) sender).getWorld()
									.getName() + "\"");
						}
	...
	private class ExpiringMetadataValue implements MetadataValue {
		...
		public org.bukkit.plugin.Plugin getOwningPlugin () {
			return DarkyenusCommandPlugin.this;
		}
