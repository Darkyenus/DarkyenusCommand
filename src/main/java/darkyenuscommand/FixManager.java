...
public final class FixManager {
    ...
    private static final class ChatFormatterListener implements Listener {
        ...
        private final List<ChatColor> hashedColors = Arrays.asList(ChatColor.AQUA, ChatColor.BLUE, ChatColor.DARK_AQUA, ChatColor.DARK_BLUE, ChatColor.DARK_GREEN, ChatColor.DARK_PURPLE, ChatColor.DARK_RED, ChatColor.GOLD, ChatColor.GREEN, ChatColor.RED, ChatColor.YELLOW);

        private ChatFormatterListener(boolean shuffle) {
            if(shuffle){
                Collections.shuffle(hashedColors,new Random(System.currentTimeMillis()/(24*60*60*1000)));//Every day, different seed!
            }
        }

        @EventHandler(ignoreCancelled = true,priority = EventPriority.HIGHEST)//Highest because this changes behavior of chat event and we want it called only after making sure that nobody wants it dead
        public void chatFormatter(AsyncPlayerChatEvent event){
            ...
            event.setFormat(playerColor+"<"+ ChatColor.WHITE+ ChatColor.BOLD+"%s"+ ChatColor.RESET+playerColor+"> "+ ChatColor.RESET+"%s");
            event.getPlayer().sendMessage(String.format(playerColor+"<"+ ChatColor.WHITE+ ChatColor.BOLD+"%s"+ ChatColor.RESET+playerColor+"> "+ ChatColor.RESET+"%s"+ ChatColor.ITALIC,event.getPlayer().getDisplayName(),event.getMessage()));
        }
    }
    ...
    private static final class BonemealGrassFix implements Listener {
        ...
        public void onBonemealUse(PlayerInteractEvent event){
            if(event.hasBlock() && event.hasItem() && event.getMaterial() == Material.INK_SACK && ((Dye)(event.getItem().getData())).getColor() == DyeColor.WHITE){
    ...
    private static final class FireFix implements Listener {
        ...
        public void onFireSpread(BlockSpreadEvent event){
            ...
            if(Math.abs(source.getX() - fire.getX()) > 1 ||
                    Math.abs(source.getY() - fire.getY()) > 1 ||
